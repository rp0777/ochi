...
function Navbar() {
  ...
  return (
    <nav className=" header backdrop-blur-md fixed z-[999] w-full px-10 py-5 md:px-5 lg:px-20 lg:py-8 flex justify-between items-center">
      {/* LOGO */}
      <div className="logo">
        <svg
          width="72"
      ...
      {/* DESKTOP VIEW */}
      <div className=" links hidden md:hidden lg:flex gap-10">
      ...
      {/* SIDEBAR OPEN */}
      {toggleNav && (
        <div className=" w-40 absolute md:overflow-auto top-2 right-2 flex flex-col justify-center items-start gap-6 md:gap-3 mt-4 py-4 px-3 bg-zinc-600 mr-3 rounded-xl">
          <button
          ...
          <div className=" w-full flex flex-col items-center justify-center gap-4 md:gap-2 px-3">
            {links.map((link, index) => (
              <a
                key={index}
                className="text-lg md:text-sm capitalize font-light bg-zinc-800 py-2 w-full flex justify-center items-center rounded-lg"
                href="#"
              >
                {link}
              </a>
